	...
	private ArrayList<Integer> processFileContents(Scanner fileReader) {
	  // To store the values of the input file
		...
		while (fileReader.hasNextLine()) {
		  ...
		  // Keep track of total number of lines (starting at 1)
	    ...
	    if (currentLine.length < WIDTH) {
	      System.out.println("Line " + current_line_number + " is too short. Padding...");
	    } else if (currentLine.length > WIDTH) {
	      System.out.println("Line " + current_line_number + " is too long. Trimming...");
	    }
	    ...
	    // Construct a row of length WIDTH
			for (var i = 0; i < WIDTH; i++) {
			  try {
			    ...
			    heatMapGridRaw.add(currentInteger);
			  } catch(ArrayIndexOutOfBoundsException e) {
			    // If line is too short, pad with -1
			    heatMapGridRaw.add(-1);
		...
		if (current_line_number < HEIGHT) {
		  ...
		  while (current_line_number < HEIGHT) {
